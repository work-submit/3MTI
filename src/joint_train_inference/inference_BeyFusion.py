...
def predict_denoised_latent(model: Difix, z: torch.Tensor, caption_enc: torch.Tensor) -> torch.Tensor:
...
def run_latent_cfg(
    ...
    pos_prompt: str,
    neg_prompt: str,
    cfg_scale_pairs: list[tuple[float, float]],
) -> dict[tuple[float, float], torch.Tensor]:
    num_views = x.shape[1]
    if num_views != 2:
        raise ValueError(f"Joint CFG expects exactly 2 views, got {num_views}.")
    ...
    for cfg_fu, cfg_sr in cfg_scale_pairs:
        cfg_per_view = torch.tensor([cfg_fu, cfg_sr], device=z.device, dtype=z.dtype)
        cfg_per_sample = cfg_per_view.view(1, num_views, 1, 1, 1)
        cfg_per_sample = cfg_per_sample.repeat(batch_size, 1, 1, 1, 1).view(batch_size * num_views, 1, 1, 1)
        z_cfg = z_neg + cfg_per_sample * (z_pos - z_neg)
        outputs[(cfg_fu, cfg_sr)] = decode_latent(model, z_cfg, skip_acts, num_views)
    return outputs


def build_cfg_scale_pairs(cfg_scales_fu: list[float], cfg_scales_sr: list[float]) -> list[tuple[float, float]]:
    if len(cfg_scales_fu) == len(cfg_scales_sr):
        return list(zip(cfg_scales_fu, cfg_scales_sr))
    if len(cfg_scales_fu) == 1:
        return [(cfg_scales_fu[0], cfg_sr) for cfg_sr in cfg_scales_sr]
    if len(cfg_scales_sr) == 1:
        return [(cfg_fu, cfg_scales_sr[0]) for cfg_fu in cfg_scales_fu]
    raise ValueError("--cfg_scales_fu and --cfg_scales_sr must have the same length, or one side must have length 1.")


def cfg_to_str(cfg: float) -> str:
    return f"cfg{str(cfg).replace('.', '')}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        required=True,
        help="CFG scales for fusion output y[0, 0], e.g. --cfg_scales_fu 1.1",
    )
    parser.add_argument(
        "--cfg_scales_sr",
        nargs="+",
        type=float,
        required=True,
        help="CFG scales for SR output y[0, 1], e.g. --cfg_scales_sr 1.05",
    )
    args = parser.parse_args()
    cfg_scale_pairs = build_cfg_scale_pairs(args.cfg_scales_fu, args.cfg_scales_sr)
    ...
    print("Fusion CFG scales:", args.cfg_scales_fu)
    print("SR CFG scales:", args.cfg_scales_sr)
    ...
    prompt_dict = {}
    if os.path.exists("./Fusion_test_prompt.txt"):
        with open("./Fusion_test_prompt.txt") as f:
            for line in f:
                if ":" in line:
    ...
    for in_path, rf_path in tqdm(zip(input_images, ref_images), total=len(input_images), desc="Processing"):
        ...
        with torch.no_grad():
            cfg_outputs = run_latent_cfg(model, x, pos_prompt, neg_prompt, cfg_scale_pairs)

        for (cfg_fu, cfg_sr), y in cfg_outputs.items():
            cfg_fu_str = cfg_to_str(cfg_fu)
            cfg_sr_str = cfg_to_str(cfg_sr)

            y_fu = (y[0, 0] * 0.5 + 0.5).clamp(0, 1).cpu()
            fu_pil = transforms.ToPILImage()(y_fu)
            fu_subdir = os.path.join(args.output_dir, f"Fu_16_{cfg_fu_str}")
            os.makedirs(fu_subdir, exist_ok=True)
            fu_pil.save(os.path.join(fu_subdir, fname))

            y_sr = (y[0, 1] * 0.5 + 0.5).clamp(0, 1).cpu()
            sr_pil = transforms.ToPILImage()(y_sr)
            sr_subdir = os.path.join(args.output_dir, f"SR_16_{cfg_sr_str}")
            os.makedirs(sr_subdir, exist_ok=True)
            sr_pil.save(os.path.join(sr_subdir, fname))
