...
# def pop_alpha_mean(model):
#     values = []
#     for module in model.modules():
#         stats = getattr(module, "csm_alpha_stats", None)
#         if stats:
#             values.extend(stats)
#             module.csm_alpha_stats = []
#     if not values:
#         return None
...
def main(args):
    ...
    dataset_train = PairedDataset(dataset_path=args.dataset_path, split="train", tokenizer=net_difix.tokenizer, prompts_file="./Train_SR_Prompt.txt")  # add prompt file
    dl_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True, num_workers=args.dataloader_num_workers)

    dataset_val = PairedDataset(dataset_path=args.dataset_path, split="test", tokenizer=net_difix.tokenizer, prompts_file="./Test_SR_Prompt.txt")     # add prompt file
    ...
    # output_dir = 'debug2'
    output_dir = 'debug_SR_spearate_cfg_woAug_10L2'
    ...
    while 1:
        ...
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                x_tgt_pred_IR = x_tgt_pred[:, 0, :, :, :]  # shape: [4, C, H, W]

                IR_gt = x_tgt[:, 0, :, :, :]

                # SR loss
                loss_lpips = net_lpips(x_tgt_pred_IR, IR_gt).mean() * args.lambda_lpips
                loss_l2 = F.mse_loss(x_tgt_pred_IR, IR_gt, reduction="mean") * args.lambda_l2
